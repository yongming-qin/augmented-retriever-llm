...
class GrpoLearningMonoBert(nn.Module):
    ...
    def compute_rewards_batch(self, questions, answers, contexts_batch):
        ...
        rewards = torch.tensor(rewards) / 100.0        
        return rewards
        ...
        for epoch in range(epochs):
            ...
            for batch_start in tqdm(range(0, len(keys), batch_size), desc=f"Epoch {epoch + 1}"):
                ...
                for key in batch_keys:
                    ...
                    contexts = ["NO CONTEXT IS NEEDED", gt_ctx] #TODO + retrieved_ctxs
                ...
                print(f"softmax(logits): {torch.softmax(logits, dim=-1)}")
                print(f"rewards: {rewards}")
                
                loss = self.loss(logits, rewards)
    ...
    def loss(self, logits, rewards):
        ...
        def margin_loss(logits, rewards):
            ...
            return F.margin_ranking_loss(score_i, score_j, target, margin=0.4)
